  ...
  if (!accounts)
    return (
      <MessageContainer
        ...
        msg="Click '+' button to add a transaction"
      />
    );
  ...
  return (
    <div className="relative overflow-x-auto bg-neutral-primary-soft shadow-xs rounded-lg border border-gray-200">
      <table className="w-full text-sm text-center rtl:text-right text-body">
        <thead
          className={`text-sm text-nowrap text-body ${nature === "asset" ? "bg-blue-100 text-blue-800" : "bg-red-100 text-red-800"}`}
        >
          <tr>
            {accounts
              ...
              .map((account) => (
                <th
                  key={account.id}
                  className="px-6 py-3 font-semibold w-40 min-w-40 max-w-40"
                >
                  {account.name}
                </th>
              ))}
